...
<%
	...
	Long id = Long.valueOf((String) session.getAttribute("idWhoRecommendedTheEntity"));
	Integer action = Integer.valueOf((String) session.getAttribute("idWhoRecommendedTheEntityAction"));
	Boolean ok = true;
	...
	try {
			PreparedStatement ps = db.conexao.prepareStatement("SELECT recommender as recommender, (SELECT nickName from Recommender WHERE id=AddRec.recommender) as nickName FROM AddRec WHERE addRec = ? AND action = ? LIMIT ?,?");
			ps.setLong(1, id);
			ps.setInt(2, action);
	...
	if(recommendations.isEmpty()) {
		...
		if(recommendations.size() > numeroDeRecomendadoresASeremExibidos) { out.println("<dd class=\"user\"><a href=\"WhoRecommendedTheEntity.jsp?id="+id+"\">More Recommenders...</a></dd>"); }
