...
<%
	...
	String buttonCount = (String) request.getParameter("buttonCount");
...
var id = $('.<%= button %>Button<%= buttonCount %>').closest("div").attr('id');

<% if(userAction.equals("UnRecommend")) { %>
$(document).ready(function(){
	$("[class*='<%= button %>Button']").one('click', function(){
	event.stopImmediatePropagation();
	$('[class*="<%= buttonDiv %>"]').load("ButtonRecommendOutro.jsp?recommender=<%= recommender %>&addRec=<%= addRec %>&action=<%= action %>&table=<%= table %>&buttonId=<%= addRec %>&UserAction=Recommend&width=<%= width %>&buttonCount=<%= buttonCount %>", function (response) { $('[class*="<%= buttonDiv %>"]').html(response); }); }); });
<% } else if(userAction.equals("Recommend")) { %>
$(document).ready(function(){
$("[class*='<%= button %>Button']").mouseover(function(){ $("[class*='<%= button %>Button']").html("UnRecommend"); $("[class*='<%= button %>Button']").addClass("<%= bu %>"); $("[class*='<%= button %>Button']").removeClass("<%= br %>"); });
$("[class*='<%= button %>Button']").mouseout(function(){ $("[class*='<%= button %>Button']").html("Recommended"); $("[class*='<%= button %>Button']").addClass("<%= br %>"); $("[class*='<%= button %>Button']").removeClass("<%= bu %>"); });
});
$(document).ready(function(){
	$("[class*='<%= button %>Button']").one('click', function(){
	event.stopImmediatePropagation();
	$('[class*="<%= buttonDiv %>"]').load("ButtonRecommendOutro.jsp?recommender=<%= recommender %>&addRec=<%= addRec %>&action=<%= action %>&table=<%= table %>&buttonId=<%= addRec %>&UserAction=UnRecommend&width=<%= width %>&buttonCount=<%= buttonCount %>", function (response) { $('[class*="<%= buttonDiv %>"]').html(response); }); }); });
<% } %>
...
<% if(userAction.equals("UnRecommend")) { %>
<button class="<%= button %>Button<%= buttonCount %> <%= bd %>">Recommend</button>
<% } else if(userAction.equals("Recommend")) { %>
<button class="<%= button %>Button<%= buttonCount %> <%= br %>">Recommended</button>
<% } %>
