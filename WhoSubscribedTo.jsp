...
<%
	...
	session.setAttribute("idWhoSubscribedTo", ""+id+"");
...
<meta name="keywords" content="Who Subscribed to the Recommender <%= recommender.getNickName() %> on Recommendation Book" />
<meta name="description" content="Who Subscribed to the Recommender <%= recommender.getNickName() %> on Recommendation Book" />
...
$(document).ready(function(){
	$(document).on("click", "tr.LoadMore", function() {
		...
		var posting = $.post("WhoSubscribedToRecs.jsp", {page: id});
...
<div class="Corpo">
	...
	<fieldset>
	<legend>Who Subscribed to the Recommender <a class="SeeAll" href="Recommender.jsp?id=<%= id %>"><%= recommender.getNickName() %></a></legend>

			<table>
			<tbody>

				<jsp:include page="WhoSubscribedToRecs.jsp?page=1" flush="true" />
