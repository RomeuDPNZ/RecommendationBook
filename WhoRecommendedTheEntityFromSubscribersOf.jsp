...
<%
	...
	session.setAttribute("idWhoRecommendedTheEntityFromSubscribersOf", ""+id+"");
	...
	session.setAttribute("recommenderWhoRecommendedTheEntityFromSubscribersOf", ""+recommender.getId()+"");
	...
	session.setAttribute("idWhoRecommendedTheEntityFromSubscribersOfAction", ""+action+"");
...
<% if(forRecommended.equals("true")) { %>
	...
	<jsp:include page="WhoRecommendedTheEntityFromSubscribersOfRecs.jsp?page=1&forRecommended=true" flush="true" />
...
<meta name="keywords" content="Who Recommended the Entity <%= recommended.getName() %> From Subscribers of <%= recommender.getNickName() %> on Recommendation Book" />
<meta name="description" content="Who Recommended the Entity <%= recommended.getName() %> From Subscribers of <%= recommender.getNickName() %> on Recommendation Book" />
...
$(document).ready(function(){
	$(document).on("click", "tr.LoadMore", function() {
		...
		var posting = $.post("WhoRecommendedTheEntityFromSubscribersOfRecs.jsp", {page: id});
...
<div class="Corpo">
	...
	<fieldset>
	<legend>Who Recommended the Entity <a class="SeeAll" href="Recommended.jsp?id=<%= id %>"><%= recommended.getName() %></a> From Subscribers of <a class="SeeAll" href="Recommender.jsp?id=<%= recommender.getId() %>"><%= recommender.getNickName() %></a></legend>

			<table>
			<tbody>

				<jsp:include page="WhoRecommendedTheEntityFromSubscribersOfRecs.jsp?page=1" flush="true" />
