...
<%
	...
	if(type.equals("Book")) {
		...
		if((readList.getId() == null) && (action.equals("ToRead"))) {
			ReadList readListInsert = new ReadList();
			readListInsert.setBook(recommendedIdShowed);
			readListInsert.setRecommender(recommenderIdLogged);
			readListInsert.setStateReadFromMySQL("ToRead");
			new ReadListDAO(db).insert(readListInsert);
			printAction = "To Read";
			activated = true;
			action = "Read";
		} else if((readList.getId() != null) && (readList.getStateReadToMySQL().equals("ToRead")) && (action.equals("Read"))) {
			readList.setStateReadFromMySQL("Read");
			new ReadListDAO(db).updateOnlyChanged(readList);
			printAction = "Read";
			activated = true;
			action = "Reset";
		} else if((readList.getId() != null) && (readList.getStateReadToMySQL().equals("Read")) && (action.equals("Reset"))) {
		...
		if(actionBackup.equals("ToRead")) {
			printAction = "To Read";
			activated = true;
			action = "Read";
		} else if(actionBackup.equals("Read")) {
			printAction = "Read";
			activated = true;
			action = "Reset";
		} else if(actionBackup.equals("Reset")) {
			printAction = "To Read";
			activated = false;
			action = "ToRead";
		}
		...
		if(watchList.getId() == null && (action.equals("ToWatch"))) {
			WatchList watchListInsert = new WatchList();
			watchListInsert.setMovie(recommendedIdShowed);
			watchListInsert.setRecommender(recommenderIdLogged);
			watchListInsert.setStateWatchFromMySQL("ToWatch");
			new WatchListDAO(db).insert(watchListInsert);
			printAction = "To Watch";
			activated = true;
			action = "Watched";
		} else if((watchList.getId() != null) && (watchList.getStateWatchToMySQL().equals("ToWatch")) && (action.equals("Watched"))) {
			watchList.setStateWatchFromMySQL("Watched");
			new WatchListDAO(db).updateOnlyChanged(watchList);
			printAction = "Watched";
			activated = true;
			action = "Reset";
		} else if((watchList.getId() != null) && (watchList.getStateWatchToMySQL().equals("Watched")) && (action.equals("Reset"))) {
			new WatchListDAO(db).delete(watchList.getId());
			printAction = "To Watch";
			activated = false;
			action = "ToWatch";
		}

		if(actionBackup.equals("ToWatch")) {
			printAction = "To Watch";
			activated = true;
			action = "Watched";
		} else if(actionBackup.equals("Watched")) {
			printAction = "Watched";
			activated = true;
			action = "Reset";
		} else if(actionBackup.equals("Reset")) {
		...
		if(listenList.getId() == null && (action.equals("ToListen"))) {
			ListenList listenListInsert = new ListenList();
			listenListInsert.setAlbum(recommendedIdShowed);
			listenListInsert.setRecommender(recommenderIdLogged);
			listenListInsert.setStateListenFromMySQL("ToListen");
			new ListenListDAO(db).insert(listenListInsert);
			printAction = "To Listen";
			activated = true;
			action = "Listened";
		} else if((listenList.getId() != null) && (listenList.getStateListenToMySQL().equals("ToListen")) && (action.equals("Listened"))) {
			listenList.setStateListenFromMySQL("Listened");
			new ListenListDAO(db).updateOnlyChanged(listenList);
			printAction = "Listened";
			activated = true;
			action = "Reset";
		} else if((listenList.getId() != null) && (listenList.getStateListenToMySQL().equals("Listened")) && (action.equals("Reset"))) {
		...
		if(actionBackup.equals("ToListen")) {
			printAction = "To Listen";
			activated = true;
			action = "Listened";
		} else if(actionBackup.equals("Listened")) {
			printAction = "Listened";
			activated = true;
			action = "Reset";
		} else if(actionBackup.equals("Reset")) {
...
<button class="<%= ButtonRecommend %> <%= br %>"><%= printAction %></button>
<% } else { %>
<button class="<%= ButtonRecommend %> <%= bd %>"><%= printAction %></button>
<% } %>
<% } else { %>
<button class="<%= ButtonRecommend %> <%= bd %>"><%= printAction %></button>
<% } %>
