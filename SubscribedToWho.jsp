...
<%
	...
	session.setAttribute("idSubscribedToWho", ""+id+"");
...
<meta name="keywords" content="<%= recommender.getNickName() %> Subscribed to Who on Recommendation Book" />
<meta name="description" content="<%= recommender.getNickName() %> Subscribed to Who on Recommendation Book" />
...
$(document).ready(function(){
	$(document).on("click", "tr.LoadMore", function() {
		...
		var posting = $.post("SubscribedToWhoRecs.jsp", {page: id});
...
<div class="Corpo">
	...
	<fieldset>
	<legend><a class="SeeAll" href="Recommender.jsp?id=<%= id %>"><%= recommender.getNickName() %></a> Subscribed to Who on Recommendation Book</legend>

			<table>
			<tbody>

				<jsp:include page="SubscribedToWhoRecs.jsp?page=1" flush="true" />
