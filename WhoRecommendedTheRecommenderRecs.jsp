...
<%
	...
	Long id = Long.valueOf((String) session.getAttribute("idWhoRecommendedTheRecommender"));
	Boolean ok = true;
	...
	try {
			PreparedStatement ps = db.conexao.prepareStatement("SELECT recommender, (SELECT nickName from Recommender WHERE id=AddRec.recommender) as nickName FROM AddRec WHERE addrec = ? and action = (SELECT id FROM Actions WHERE action = 'Recommended the Recommender') LIMIT ?,?");
			ps.setLong(1, id);
