...
<%
	...
	Long id = Long.valueOf((String) session.getAttribute("idWhoRecommendedTheEntityFromRecommendersIAmSubscribed"));
	Long recommender = Long.valueOf((String) session.getAttribute("recommenderWhoRecommendedTheEntityFromRecommendersIAmSubscribed"));
	Integer action = Integer.valueOf((String) session.getAttribute("idWhoRecommendedTheEntityFromRecommendersIAmSubscribedAction"));
	Boolean ok = true;
	...
	try {
			PreparedStatement ps = db.conexao.prepareStatement("SELECT AddRec.recommender as recommender, (SELECT nickName from Recommender WHERE id=AddRec.recommender) as nickName FROM AddRec WHERE addRec = ? AND action = ? AND AddRec.recommender IN (SELECT recommender FROM Subscription WHERE subscriber = ?) LIMIT ?,?");
			ps.setLong(1, id);
			ps.setInt(2, action);
	...
	if(recommendations.size() > numeroDeRecomendadoresASeremExibidos) { out.println("<dd class=\"user\"><a href=\"WhoRecommendedTheEntityFromRecommendersIAmSubscribed.jsp?id="+id+"&recommender="+recommender+"\">More Recommenders...</a></dd>"); }
